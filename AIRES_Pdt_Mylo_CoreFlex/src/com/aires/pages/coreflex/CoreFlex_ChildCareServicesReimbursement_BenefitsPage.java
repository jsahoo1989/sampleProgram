...
public class CoreFlex_ChildCareServicesReimbursement_BenefitsPage extends BenefitPage {

	public CoreFlex_ChildCareServicesReimbursement_BenefitsPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(how = How.XPATH, using = "//h5[contains(text(),'Child Care Services Reimbursement')]/ancestor::a[contains(@href,'collapse')]")
	private WebElement _formChildCareServicesReimbursement;

	@FindBy(how = How.XPATH, using = "//h5[contains(text(),'Child Care Services Reimbursement')]")
	private WebElement _headerChildCareServicesReimbursement;
	...
	@FindBy(how = How.XPATH, using = "//h4[@class='card-title'][contains(text(),'Child Care Services Reimbursement')]")
	...
	CoreFlex_LifestyleBenefitsData lifestyleBenefitData = FileReaderManager.getInstance().getCoreFlexJsonReader()
			.getLifestyleBenefitDataList(COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT);
	...
	public boolean verifyPageNavigation(String expectedPageName) {
		return CoreFunctions.verifyElementOnPage(driver, _headerPage,
				COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT, expectedPageName, expectedPageName, true);
	}
	...
	public void selectAndFillBenefitsAndSubBenefitDetails(String benefitType, String subBenefitNames,
		...
		CoreFunctions.explicitWaitTillElementVisibility(driver, _headerPageName,
				COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT_BENEFITS_PAGE);
				...
				if (CoreFunctions.isElementExist(driver, getElementByName(subBenefit.trim()), 5)) {
					...
					Reporter.addStepLog(
							MessageFormat.format(COREFLEXConstants.SUB_BENEFIT_FORM_NOT_DISPLAYED, CoreConstants.FAIL,
									subBenefit, COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT_BENEFITS_PAGE));
	...
	public void fillSubBenefit(String subBenefit, String benefitType) {
		...
		case COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT:
			...
			fillChildCareServicesReimbursementSubBenefitForm(COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT);
			break;
		default:
	...
	/**
	 * Method to fill Child Care Services Reimbursement subBenefit form
	 * 
	 * @param subBenefitFormName
	 */
	private void fillChildCareServicesReimbursementSubBenefitForm(String subBenefitFormName) {
		try {
			CoreFunctions.selectItemInListByText(driver, _radioBtnGrossUp,
					lifestyleBenefitData.childCareServicesReimbursement.grossUp, true);
			CoreFunctions.selectItemInListByText(driver, _radioBtnCandidateSelection,
					lifestyleBenefitData.childCareServicesReimbursement.reimbursedBy, true);
			if (lifestyleBenefitData.childCareServicesReimbursement.reimbursedBy
					.equalsIgnoreCase(COREFLEXConstants.OTHER)) {
				CoreFunctions.clearAndSetText(driver, _inputReimbursedByOther,
						lifestyleBenefitData.childCareServicesReimbursement.reimbursedByOther);
			}
			CoreFunctions.clearAndSetText(driver, _txtAreaComment,
					lifestyleBenefitData.childCareServicesReimbursement.comment);
		} catch (Exception e) {
	...
	public WebElement getElementByName(String elementName) {
		...
		switch (elementName) {
		case COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT:
			element = _formChildCareServicesReimbursement;
			break;
		default:
			Assert.fail(MessageFormat.format(COREFLEXConstants.ELEMENT_NOT_FOUND, CoreConstants.FAIL));
		}
	...
	public void selectBenefitTypeAndFillMandatoryFields(String benefitType, String multipleBenefitSelection,
		...
		Benefit childCareServicesReimBenefit = coreBenefits.stream()
				.filter(b -> b.getBenefitType().equals(COREFLEXConstants.CHILD_CARE_SERVICES_REIMBURSEMENT)).findAny()
				.orElse(null);
		switch (benefitType) {
		case COREFLEXConstants.CORE:
			CoreFunctions.clickElement(driver, _textCore);
			fillManadatoryDetails(benefitType, multipleBenefitSelection,
					childCareServicesReimBenefit.getBenefitDisplayName(),
					childCareServicesReimBenefit.getBenefitAmount(), childCareServicesReimBenefit.getBenefitDesc(),
					aireManagedService);
			break;
	...
	private void verifyBenefitsMandatoryDetails(String benefitType, String multipleBenefitSelection, String flexPoints,
		...
		case COREFLEXConstants.CORE:
			...
			verifyManadatoryDetails(benefitType, multipleBenefitSelection,
					childCareServicesReimBenefit.getBenefitDisplayName(),
					childCareServicesReimBenefit.getBenefitAmount(), childCareServicesReimBenefit.getBenefitDesc(),
					paymentOption, airesManagedService);
			break;
		case COREFLEXConstants.FLEX:
			CoreFunctions.clickElement(driver, _textFlex);
...
//			}
	...
	/**
	 * Method to verify Child Care Services Reimbursement subBenefit form
	 */
	private void verifyChildCareServicesReimbursementSubBenefitForm(String formName) {
		try {
			CoreFunctions.verifyRadioButtonSelection(driver, _radioGrossUpLabelList, _radioGrossUpButtonList,
					lifestyleBenefitData.childCareServicesReimbursement.grossUp, COREFLEXConstants.GROSS_UP);
			CoreFunctions.verifyRadioButtonSelection(driver, _radioReimbursedByLabelList, _radioReimbursedByButtonList,
					lifestyleBenefitData.childCareServicesReimbursement.reimbursedBy, COREFLEXConstants.REIMBURSED_BY);
			if (lifestyleBenefitData.childCareServicesReimbursement.reimbursedBy
					.equalsIgnoreCase(COREFLEXConstants.OTHER)) {
			...
			CoreFunctions.verifyText(_txtAreaComment.getDomProperty("value"),
					lifestyleBenefitData.childCareServicesReimbursement.comment, COREFLEXConstants.COMMENT);
