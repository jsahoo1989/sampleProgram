...
public class CoreFlex_FurnitureRental_BenefitsPage extends BenefitPage {

	public CoreFlex_FurnitureRental_BenefitsPage(WebDriver driver) {
		super(driver);
	}
	...
	@FindBy(how = How.XPATH, using = "//h5[contains(text(),'Furniture Rental')]/ancestor::a[contains(@href,'collapse')]")
	private WebElement _formFurnitureRental;

	@FindBy(how = How.XPATH, using = "//h5[contains(text(),'Furniture Rental')]")
	private WebElement _headerFurnitureRental;
	...
	@FindBy(how = How.XPATH, using = "//h4[@class='card-title'][contains(text(),'Furniture Rental')]")
	...
	CoreFlex_SettlingInBenefitsData settlingInBenefitData = FileReaderManager.getInstance().getCoreFlexJsonReader()
			.getSettlingInBenefitDataList(COREFLEXConstants.FURNITURE_RENTAL);
	...
	public boolean verifyPageNavigation(String expectedPageName) {
		return CoreFunctions.verifyElementOnPage(driver, _headerPage, COREFLEXConstants.FURNITURE_RENTAL,
	...
	public void selectAndFillBenefitsAndSubBenefitDetails(String benefitType, String subBenefitNames,
		...
		CoreFunctions.explicitWaitTillElementVisibility(driver, _headerPageName,
				COREFLEXConstants.FURNITURE_RENTAL_BENEFITS_PAGE);
			...
			for (String subBenefit : subBenefitNamesList) {
				...
				if (CoreFunctions.isElementExist(driver, getElementByName(subBenefit.trim()), 5)) {
					...
					Reporter.addStepLog(MessageFormat.format(COREFLEXConstants.SUB_BENEFIT_FORM_NOT_DISPLAYED,
							CoreConstants.FAIL, subBenefit, COREFLEXConstants.FURNITURE_RENTAL_BENEFITS_PAGE));
	...
	public void fillSubBenefit(String subBenefit, String benefitType) {
		...
		case COREFLEXConstants.FURNITURE_RENTAL:
			expandSubBenefitIfCollapsed(getElementByName(COREFLEXConstants.FURNITURE_RENTAL));
			if (benefitType.equals(COREFLEXConstants.FURNITURE_RENTAL)) {
				CoreFunctions.clickElement(driver, _headerFurnitureRental);
			}
			fillFurnitureRentalSubBenefitForm(COREFLEXConstants.FURNITURE_RENTAL);
			break;
		default:
	...
	/**
	 * Method to fill Furniture Rental subBenefit form
	 * 
	 * @param subBenefitFormName
	 */
	private void fillFurnitureRentalSubBenefitForm(String subBenefitFormName) {
		try {
			CoreFunctions.clickElement(driver, _selectDuration);
			CoreFunctions.selectItemInListByText(driver, _selectDurationOptions,
					settlingInBenefitData.furnitureRental.duration, true);
			CoreFunctions.clearAndSetText(driver, _txtAreaComment, settlingInBenefitData.furnitureRental.comment);
			CoreFunctions.selectItemInListByText(driver, _radioBtnGrossUp,
					settlingInBenefitData.furnitureRental.grossUp, true);
			CoreFunctions.selectItemInListByText(driver, _radioBtnCandidateSelection,
					settlingInBenefitData.furnitureRental.reimbursedBy, true);
			if (settlingInBenefitData.furnitureRental.reimbursedBy.equalsIgnoreCase(COREFLEXConstants.OTHER)) {
				CoreFunctions.clearAndSetText(driver, _inputReimbursedBy,
						settlingInBenefitData.furnitureRental.reimbursedByOther);
			}
		} catch (Exception e) {
	...
	public void selectBenefitTypeAndFillMandatoryFields(String benefitType, String multipleBenefitSelection,
		...
		Benefit furnitureRentalBenefit = coreBenefits.stream()
				.filter(b -> b.getBenefitType().equals(COREFLEXConstants.FURNITURE_RENTAL)).findAny().orElse(null);
		switch (benefitType) {
		case COREFLEXConstants.CORE:
			CoreFunctions.clickElement(driver, _textCore);
			fillManadatoryDetails(benefitType, multipleBenefitSelection, furnitureRentalBenefit.getBenefitDisplayName(),
					furnitureRentalBenefit.getBenefitAmount(), furnitureRentalBenefit.getBenefitDesc(),
					aireManagedService);
			break;
	...
	private void verifyBenefitsMandatoryDetails(String benefitType, String multipleBenefitSelection, String flexPoints,
		...
		case COREFLEXConstants.CORE:
			...
			verifyManadatoryDetails(benefitType, multipleBenefitSelection,
					furnitureRentalBenefit.getBenefitDisplayName(), furnitureRentalBenefit.getBenefitAmount(),
					furnitureRentalBenefit.getBenefitDesc(), paymentOption, airesManagedService);
			break;
		case COREFLEXConstants.FLEX:
			CoreFunctions.clickElement(driver, _textFlex);
	...
	private void verifySubBenefitDetails(String subBenefit, String benefitType) {
		...
		case COREFLEXConstants.FURNITURE_RENTAL:
			...
			verifyFurnitureRentalSubBenefitForm(COREFLEXConstants.FURNITURE_RENTAL);
			break;
		default:
			Assert.fail(MessageFormat.format(COREFLEXConstants.ELEMENT_NOT_FOUND, CoreConstants.FAIL));
		}
	...
	private void verifyFurnitureRentalSubBenefitForm(String formName) {
		try {
			...
			CoreFunctions.verifyRadioButtonSelection(driver, _radioReimbursedByLabelList, _radioReimbursedByButtonList,
					settlingInBenefitData.furnitureRental.reimbursedBy, COREFLEXConstants.REIMBURSED_BY);
			if (settlingInBenefitData.furnitureRental.reimbursedBy.equalsIgnoreCase(COREFLEXConstants.OTHER)) {
				CoreFunctions.verifyText(_inputReimbursedBy.getDomProperty("value"),
						settlingInBenefitData.furnitureRental.reimbursedByOther, COREFLEXConstants.REIMBURSED_BY_OTHER);
				CoreFunctions.highlightObject(driver, _inputReimbursedBy);
			}
			CoreFunctions.verifyText(_txtAreaComment.getDomProperty("value"),
					settlingInBenefitData.furnitureRental.comment, COREFLEXConstants.COMMENT);
	...
	public boolean verifyFlexBenefitCardStatusAfterInitialActualization(int index, String expectedEstimatedDate) {
		return (CoreFunctions.isElementExist(driver,
				...
				&& (!CoreFunctions.isElementExist(driver, CoreFunctions.findSubElement(
						flexCardPanelList.get(index), flexCardStartingSoonStatusBy), 3)));
	}
