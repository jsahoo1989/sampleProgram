...
public class MobilityX_DashboardHomePage extends Base{
	...
	// Employee First Name
	@FindBy(how = How.XPATH, using = "//span[@id='fname']/input[@id='fname::content']")
	private WebElement _txtBoxEmpFirstName;

	// Employee Last Name
	@FindBy(how = How.XPATH, using = "//span[@id='lname']/input[@id='lname::content']")
	...
	public void switchToiFrame_Authorization() {
		try {
			...
			Log.info("loading pop-up id=="+CoreFunctions.isElementByLocatorExist(driver, _loadingPopUpByLocator, 10));
	...
	public void selectAuthorizationOptionForEmployee(String option) {
		try {
			...
			Assert.assertEquals(CoreFunctions.getElementText(driver, _titleText_AuthorizationNextStep),
					...
					MobilityXConstants.AUTHORIZATION_FORM_TITLE + MobilityXConstants.TITLE + MobilityXConstants.IS_NOT_DISPLAYED);
	...
	public void fillGlobalReloFamilyInfo(GlobalReloFamilyInfo familyInfo) {
		try {
			BusinessFunctions.selectRadioAsPerLabelText(driver, _radioBtnMaritalStatus,
					familyInfo.maritalStatus);	
			BusinessFunctions.selectValueFromDropdown(_drpDownGreenCardHolder, familyInfo.greenCardHolder);
			//CoreFunctions.clearAndSetText(driver, _txtBoxStartDateInNewLocation, familyInfo.startDateInNewLocation);
			BusinessFunctions.selectRadioAsPerLabelText(driver, _radioBtnIsSpouseEmployed,
					familyInfo.isSpouseEmployed);			
		} catch (Exception e) {
			Log.info(CoreConstants.ERROR + e);
		}
	}
	...
	public boolean authFormExists(String templateName) {
		...
		if (_isDisplayed) {
			...
			Assert.assertEquals(CoreFunctions.getElementText(driver, _titleAuthorizationForm),
					MobilityXConstants.AUTHORIZATION_FORM_TITLE_TEXT,
					MobilityXConstants.AUTHORIZATION_FORM_TITLE + MobilityXConstants.TITLE + MobilityXConstants.NOT_EXIST);
			BusinessFunctions.selectValueFromDropdown(_drpdown_AuthorizationFormTemplate,
					templateName);
	...
	public boolean verifySuccessDialogDisplayed() {
		try {
			if (CoreFunctions.isElementByLocatorExist(driver, _growlMessageByLocator, 25)) {
				Reporter.addStepLog(CoreConstants.PASS + CoreConstants.VRFIED_THAT + MobilityXConstants.SUCCESS_MESSAGE_TEXT
