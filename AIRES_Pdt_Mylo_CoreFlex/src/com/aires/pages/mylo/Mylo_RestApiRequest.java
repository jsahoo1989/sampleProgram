...
public class Mylo_RestApiRequest extends Base {
	...
	public void createPayoadMapForCreateAssignment() {
	...
	public void setCreateNewAssignmentPayload() {
		createPayoadMapForCreateAssignment();
	...
	public String getResponseFieldValue(JSONObject responseData, String fieldName1, String fieldName2,
			int index) {
		String requiredValue;
		checkFieldExistInResponse(responseData, fieldName1);
		switch (fieldName1) {
		case MYLOConstants.PRIMARY_CONTACT_CARD_OBJECT:
			requiredValue = responseData.getJSONObject(MYLOConstants.PRIMARY_CONTACT_CARD_OBJECT).get(fieldName2).toString();
			break;
		case MYLOConstants.ASSIGNMENT_EMPLOYEE_RESPONSE_LIST:
			requiredValue = responseData.getJSONArray(MYLOConstants.ASSIGNMENT_EMPLOYEE_RESPONSE_LIST).getJSONObject(index)
					.get(fieldName2).toString();
			break;
		default:
	...
	private void checkFieldExistInResponse(JSONObject responseData, String fieldName) {
		if(responseData.keySet().contains(fieldName))
			Reporter.addStepLog(MessageFormat.format(MYLOConstants.FIELD_NAME_EXIST_IN_RESPONSE,
	...
	public boolean validateGetAssignmentResponse(JSONObject responseData) {
		...
		if(responseData.get("companyId").toString().equals(createAssignmentPayLoadMap.get("clientId"))&&
				...
				responseData.get("assignmentOfficeCode").toString().equals(createAssignmentPayLoadMap.get("officeCode"))&&
				responseData.get("journeyTypeCode").toString().equals(createAssignmentPayLoadMap.get("journeyTypeCode"))&&
